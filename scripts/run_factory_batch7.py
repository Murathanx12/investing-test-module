"""Run Strategy Factory batch 7 — explore tier. Frozen in STRATEGY_FACTORY.md
BEFORE running. Usage: .venv\Scripts\python -m scripts.run_factory_batch7
"""
...
def main() -> None:
    ...
    table = run_batch(panel, build_batch7(panel))
    table.to_csv(OUT / "batch7_summary.csv", index=False)
    with open(OUT / "batch7_summary.txt", "w") as fh:
