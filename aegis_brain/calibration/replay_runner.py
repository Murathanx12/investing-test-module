...
def load_candidates(factory_dir: Path) -> pd.DataFrame:
    ...
    Contaminated rows are excluded from the one-shot (their confirm window is
    already burned) and reported separately."""
    ...
    return allrows.drop_duplicates(["signal", "segment"], keep="first")
...
def stage_a(cands: pd.DataFrame, cdfs: dict[str, np.ndarray],
    ...
    live = cands[~cands.contaminated.astype(bool)].copy()
    excluded = cands[cands.contaminated.astype(bool)]
    ...
    accounting = {
        ...
        "n_excluded_contaminated": int(len(excluded)),
        "bh_q": Q, "top_n": TOP_N,
...
def main() -> None:
    ...
    cands = load_candidates(FACTORY_DIR)
